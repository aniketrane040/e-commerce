...
const Cart = ({ cart , onUpdateQty , onRemoveFromCart , onEmptyCart }) => {
    ...
    const EmptyCart = () => (
        <>
            <Typography variant="subtitle1">You dont have any Item in your Shopping Cart</Typography>
            ...
            <Grid container spacing={3}>
                {
                    cart.line_items.map((item) => (
                        <Grid item xs={12} sm={4} key={item.id}>
                            <CartItem item={item} onUpdateQty={onUpdateQty} onRemoveFromCart={onRemoveFromCart} />
                        </Grid>
                    ))
                }
            </Grid>
